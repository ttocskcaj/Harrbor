...
public class RadarrClient : IRadarrClient
{
    ...
    public async Task<IReadOnlyList<QueueItem>> GetQueueAsync(CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            var url = $"/api/v3/queue?page={page}&pageSize={pageSize}&includeMovie=false&includeUnknownMovieItems=true";
        ...
        _logger.LogDebug("Radarr queue returned {Count} items", items.Count);
        return items;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (requestBody != null)
        {
            ...
            _logger.LogError(
                "Radarr API request failed: {Method} {Url} returned {StatusCode}. Request body: {RequestBody}. Response body: {ResponseBody}",
                method, url, (int)response.StatusCode, requestBodyJson, responseBody);
        }
        else
        {
            _logger.LogError(
                "Radarr API request failed: {Method} {Url} returned {StatusCode}. Response body: {ResponseBody}",
